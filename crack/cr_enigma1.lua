...
local text=io.read("*a"):clean()

local key,english,verbose=",1-1-1,B,123,1,",false,false
key=arg[1] or key

local max,max_key,max_decrypt=0,0,""
local list={}
local test="ABCDEFGHIJKLMNOPQRSTUVWXYZ"
for a in test:gmatch("%a") do
for b in test:gmatch("%a") do
for c in test:gmatch("%a") do
	local radstellung=a..b..c
	local enigma = Enigma.new(radstellung..key, verbose)
	local decrypt=enigma:crypt(text)
--io.stderr:write(radstellung, " ", decrypt, "\n")
	local rating=gram_rating(decrypt)
	list[#list+1]={stellung=radstellung,weight=rating,text=decrypt}
	if rating>max then
		max=rating
		max_decrypt=decrypt
		max_key=radstellung
...
local percent=arg[2] or tonumber(arg[2]) or 0.7
for _,v in ipairs(list) do 
	if v.weight<percent*max then break end
	io.stderr:write(string.format("%s %1.2f %s\n", v.stellung, v.weight, v.text))
end
--best guess
...
--time luajit -e 'i=1 txt="ABCDEFGHIJKLMNOPQRSTUVWXYZ" for a in txt:gmatch("%a") do for b in txt:gmatch("%a") do for c in txt:gmatch("%a") do io.write(i,":",a,b,c, " ") i=i+1 end end end '
